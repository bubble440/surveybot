...
foreach ($account in $accounts) {
    ...
    # -- Logs ------------------------------------------------------------------
    # AppRotateSeconds/AppRotateBytes sont des SEUILS MINIMUMS avant que NSSM
    # n'accepte de roter le fichier existant au demarrage (comportement par
    # defaut, AppRotateOnline non positionne = rotation uniquement au demarrage
    # du service, jamais en cours d'execution). Avec 86400s/10 Mo, un bot qui
    # redemarre plusieurs fois en moins de 24h (crash-loop, EXIT_SOFT_RESTART)
    # n'atteint jamais ces seuils : NSSM APPEND dans le meme fichier au lieu de
    # roter -> plusieurs sessions ("BOOT: surveybot starting") melangees dans
    # un seul fichier, diagnostic impossible a correler par session.
    # A 0, ces deux seuils sont desactives (valeur documentee NSSM) : rotation
    # inconditionnelle a CHAQUE demarrage de process -> 1 fichier par session.
    # La purge des anciens fichiers rotes (conservation des 10 dernieres
    # sessions) est geree cote Python, voir launch.py::_purge_old_session_logs.
    & nssm set $svcName AppStdout        $logStdout
    & nssm set $svcName AppStderr        $logStderr
    & nssm set $svcName AppRotateFiles   1
    & nssm set $svcName AppRotateSeconds 0
    & nssm set $svcName AppRotateBytes   0
